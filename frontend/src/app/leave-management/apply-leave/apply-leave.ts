...
export class ApplyLeave implements OnInit {
  ...
  get leaveTypes(): string[] {
    let gender = '';
    if (this.isEmployee && this.auth.user) {
      gender = (this.auth.user as any).gender || '';
    }
    if (!gender && this.newName && this.employees.length > 0) {
      const emp = this.employees.find(e => e.name.toLowerCase() === this.newName.toLowerCase() || (e.email && e.email.toLowerCase() === this.newName.toLowerCase()));
      if (emp?.gender) {
        gender = emp.gender;
      }
    }
